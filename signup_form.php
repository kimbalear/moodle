...
function get_random_string($length = 20){
    ...
    for ($i=1;$i<=$max;$i++){
            $nickname .= $cons[rand(0,count($cons))];
            $nickname .= $voca[rand(0,count($voca))];
    }
    $nickname .= "@kassai.org";
...
class login_signup_form extends moodleform implements renderable, templatable {
    function definition() {
        ...
        $mform->addElement('text', 'email', get_string('email'), "maxlength='100' size='25' value='$random_mail'");
        $mform->setType('email', core_user::get_property_type('email'));
        $mform->addRule('email', get_string('missingemail'), 'required', null, 'client');
        $mform->setForceLtr('email');

        $mform->addElement('text', 'email2', get_string('emailagain'), "maxlength='100' size='25' value='$random_mail'");
